...
function updateList(){  
  // nếu đang ở một section menu (favorites, history, ...) thì render theo section đó  
  if(activeSection){  
  ...
  // sort: default -> newest first (by uploadISO)  
  const opt = document.getElementById("sortFilter") ? document.getElementById("sortFilter").value : "none";  
  
  if(opt === "newest" || opt === "none"){  
    filtered.sort((a, b) => {  
      const ta = (new Date(a.uploadISO || 0)).getTime() || 0;  
      const tb = (new Date(b.uploadISO || 0)).getTime() || 0;  
      return tb - ta;  
    });  
  } else if(opt === "oldest"){  
    filtered.sort((a, b) => {  
...
/* handleEnter with stacking (cộng dồn VIP) */  
...
function renderWheelFor(cost){  
  ...
  if(!cost || !spinWheelDefs[cost]) {  
    if(wheel) wheel.style.background = 'linear-gradient(180deg,#eee,#ddd)';  
...
function openSpinSection(){  
  ...
  if(!selectedSpin){ selectedSpin='100'; if(spin100El) spin100El.classList.add('selected'); }  
...
[spin50El, spin100El, spin150El].forEach(el=>{  
  ...
  el.addEventListener('click', ()=> {  
    [spin50El, spin100El, spin150El].forEach(x=>{ if(x) x.classList.remove('selected'); });  
...
function init(){  
  ...
  if(searchInput) searchInput.addEventListener("input", ()=>{ if(!activeSection) updateList(); });  
...
/* === DAILY CHECKIN INJECT (kept from your file) === */  
(function(){  
  ...
  if(!document.getElementById('my_daily_modal')){  
    ...
    document.getElementById('my_claim_btn').addEventListener('click', ()=>{  
      ...
      refreshMenuBadge();  
      if(res.ok){  
        setTimeout(()=>{ alert('Bạn nhận được:\n' + (res.grant.join('\n'))); }, 40);  
      } else {  
